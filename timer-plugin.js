	...
	require(["CerosSDK"], function (CerosSDK) {
		CerosSDK.findExperience()
			...
			.done(function (experience) {
				const pageChangedFunction = function (page) {
				...
				const animationToggleCallback = (anim, bool) =>
					document
						.getElementById(anim.id)
						.setAttribute("allow-timer-animation", bool)
				...
				const animationStartedCallback = (a) => {
					...
					if (
						textBox.getAttribute("allow-timer-animation") == "yes"
					) {
						// checking and setting delay
						let keyframes = textBox
							.getAnimations()[0]
							.effect.getKeyframes()
						let animDuration = parseFloat(
							getComputedStyle(textBox).animationDuration
						)
						let animDelay =
							keyframes.length > 2
								? animDuration * 1000 * keyframes[1].offset
								: 0
						...
						let decimal = new TagToObject("decimal", ".")
						for (let tag of tags) {
							for (let ar of arr) {
						...
						// Set decimal separator based on tag
						let decimalSeparator = decimal.value === "," ? "," : "."
							...
							.animate(
								...
								},
								{
									...
									step: function () {
										...
										if (
											this.countNumber == timer[2] &&
											endText.value != ""
										)
											txt.innerText =
												number + endText.value
									},
									complete: function () {
										let number = getNumber(this.countNumber)
										if (endText.value != "")
